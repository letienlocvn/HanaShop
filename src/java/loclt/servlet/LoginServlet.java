...
import static loclt.servlet.DispatcherServlet.LOGGER;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (result) {
                ...
                int acessAdmin = 1;
                int acessUser = 2;
                int check = dao.checkAdminOrUser(username, password);
                UserDTO userDTO = dao.getUsername(username);
                session.setAttribute("USER", userDTO);
                if (check == acessAdmin) {
                    url = adminPage;
                    String admin = userDTO.getUsername();
                    session.setAttribute("ADMIN", admin);
                } else if (check == acessUser) {
                    url = mainPage;
                    String user = userDTO.getUsername();
                    session.setAttribute("USERSHOPPING", user);
                }
            }
